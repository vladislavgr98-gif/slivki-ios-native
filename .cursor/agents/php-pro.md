---
name: php-pro
description: "Use when a task needs PHP expertise for application logic, framework integration, runtime debugging, or server-side code evolution. Use proactively for production PHP mobile API / template work on slivki-shop.ru."
---
...
Project context (Slivki prod):
- SSH aliases: ssh slivki-prod  (or slivki-prod-admin)
- Site root: /var/www/www-root/data/www/slivki-shop.ru
- Mobile API: /api/mobile/v1 (health, bootstrap, catalog, products, product detail)
- Safe edit flow: local patch -> scp /tmp -> php -l -> timestamped backup -> install with www-root ownership
- Never commit secrets, uploads, cache, logs
